    ...
    if p_flag==1:
        if Flow_i!=0:
            if r_v-(Flow_i+delta)>0:
                val = (gamma*to_am*np.log((Flow_i+delta)/(r_v-(Flow_i+delta)))*(Flow_i+delta)+beta*to_am*(Flow_i+delta)-(gamma*to_am*np.log(Flow_i/(r_v-Flow_i))*Flow_i+beta*to_am*Flow_i))/delta #delta流した時の1単位あたりのコスト
            else:
                val= np.infty
        else:
            if r_v-(Flow_i+delta)>0:
...
for setting_tmp in range(num_simulation):
    ...
    W=W_low+(W_up-W_low)*np.random.rand(m,n)
    ...
    while delta>0.0000001:
        ...
        while len(list(*np.where(excess >= delta)))>0 and len(list(*np.where(excess <= -delta)))>0:
            #print('here',list(*np.where(excess >= delta))[0],list(*np.where(excess <= -delta))[0])
...
#Outputs results
with open('results_synthetic/result_m=%d_n=%d_T=%d_bu=%dto%d_BOruntime=%d_RSruntime=%d_simulations=%d_Wlow=%f_Wup=%f.csv' %(m,n,T,rand_low,rand_up,bo_run_time,rs_run_time,num_simulation,W_low,W_up), mode='w') as f_tmp:
